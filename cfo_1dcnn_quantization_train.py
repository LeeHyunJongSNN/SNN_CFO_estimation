...
class QuantNet1D(nn.Module):
    """
    QAT-ready 1D CNN:
    - QuantStub/DeQuantStub for static quantization.
    - Explicit ReLU modules to enable module fusion.
    """
    def __init__(self):
        super().__init__()
    ...
    def fuse_model(self):
        # Fusing improves quantized accuracy and performance.
    ...
    def forward(self, x):
        ...
        x = self.conv(x)      # (conv+bn+relu) fused after fuse_model()
        x = self.pool(x)

        x = self.flatten(x)
        x = self.fc1(x)       # (linear+relu) fused after fuse_model()
        x = self.fc2(x)
        x = self.fc3(x)
...
# Build QAT model
net = QuantNet1D().to(device)

# Fuse (must be in eval mode), then prepare QAT
net.eval()
net.fuse_model()
net.train()
