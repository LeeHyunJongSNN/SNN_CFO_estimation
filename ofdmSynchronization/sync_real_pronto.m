...
% Rx signals
rxSignals = readmatrix("WiFi_10MHz_record_wireless(NLOS)_1.2m.txt");
...
while (searchOffset+minPktLen)<=rxWaveformLen
    ...
    lltf_c = rxWaveform(pktOffset+(ind.LLTF(1):ind.LLTF(2)),:);
    lltf_c = detrend(lltf_c - mean(lltf_c));
    ltf_c = py.numpy.array(lltf_c);
    coarseFreqOffset = pyrunfile("cfo_estimate_prontol.py", "result", mat_input=lltf_c);
