...
class SlimmableNet1D(nn.Module):
    """
    Slimmable 1D CNN (single shared network executable at multiple widths).
    - Uses prefix slicing of channels/neurons based on a global width multiplier.
    - Uses switchable BatchNorm for the Conv layer (one BN per width).
    """
    def __init__(self, widths):
        ...
        self.conv = nn.Conv1d(1, self.max_c, 3, padding=1, stride=1, bias=True)
        self.bn_list = nn.ModuleList([nn.BatchNorm1d(max(1, int(self.max_c * w))) for w in self.widths])

        self.pool = nn.MaxPool1d(2, 2)
        self.flatten = nn.Flatten()

        # Note: these Linear layers store the MAX width weights; forward() slices prefix rows/cols.
        self.fc1 = nn.Linear(self.max_c * 80, self.max_fc1)
    ...
    def set_width(self, width: float):
        # snap to nearest predefined width (robust for float parsing)
        width = float(width)
    ...
    def forward(self, x):
        ...
        # Conv1d with sliced output channels (compute only prefix filters)
        ...
        x = F.relu(x)
        x = self.pool(x)
        x = self.flatten(x)  # (B, c_out*80)

        in_dim = x.shape[1]  # c_out*80

        # FC1: slice rows (outputs) and cols (inputs)
...
net = SlimmableNet1D(WIDTHS).to(device)
net.set_width(1.0)  # default
...
SAVE_PATH = "/home/leehyunjong/PycharmProjects/Machine_Learning/SNN/CFO/models/incomplete/" \
            "cfo_1dcnn_wireless_slimmable.pth"
...
for epoch in range(n_epochs):
    ...
    for inputs, labels in train_loader:
        ...
        # sample width for this iteration
...
# Save best model (robust even if early stop is disabled)
if best_model_state is not None:
    net.load_state_dict(best_model_state, strict=True)
torch.save(net.state_dict(), SAVE_PATH)
print(f"Saved slimmable 1D CNN model to: {SAVE_PATH}")
